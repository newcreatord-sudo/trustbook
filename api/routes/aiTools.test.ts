...
describe('ai-tools routes (notes + floor tools)', () => {
  ...
  it('maps ai_*_disabled to 403', async () => {
    rpcMock.mockResolvedValueOnce({ data: null, error: { message: 'ai_floor_plan_read_disabled' } })
    const res = await request(app as never)
      .get('/api/ai-tools/floor-plan/bundle')
      .query({ businessId: '11111111-1111-4111-8111-111111111111', agentId: 'trustbook_director_ai' })
      .set('Authorization', 'Bearer test.jwt')
      .expect(403)
    expect(res.body?.success).toBe(false)
  })
