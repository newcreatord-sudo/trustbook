...
describe('ai-tools routes (notes + floor tools)', () => {
  ...
  it('calls RPC ai_get_business_day_summary', async () => {
    rpcMock.mockResolvedValueOnce({ data: { counts: { total: 1 } }, error: null })
    const res = await request(app as never)
      .get('/api/ai-tools/day-summary')
      .query({
        businessId: '11111111-1111-4111-8111-111111111111',
        day: '2026-05-16',
      })
      ...
      .expect(200)
    expect(res.body?.success).toBe(true)
    expect(rpcMock).toHaveBeenCalledWith('ai_get_business_day_summary', expect.any(Object))
  })

  it('calls RPC ai_list_bookable_slots_for_service_day', async () => {
    rpcMock.mockResolvedValueOnce({ data: [{ start_at: '2026-05-16T10:00:00Z', end_at: '2026-05-16T10:30:00Z' }], error: null })
    const res = await request(app as never)
      .get('/api/ai-tools/slots/bookable')
      .query({
        businessId: '11111111-1111-4111-8111-111111111111',
    ...
    expect(res.body?.success).toBe(true)
    expect(Array.isArray(res.body?.rows)).toBe(true)
    expect(rpcMock).toHaveBeenCalledWith('ai_list_bookable_slots_for_service_day', expect.any(Object))
  })
  ...
  it('maps ai_booking_operator_disabled to 403 on day-summary', async () => {
    rpcMock.mockResolvedValueOnce({ data: null, error: { message: 'ai_booking_operator_disabled' } })
    const res = await request(app as never)
      .get('/api/ai-tools/day-summary')
      .query({ businessId: '11111111-1111-4111-8111-111111111111', day: '2026-05-16' })
      .set('Authorization', 'Bearer test.jwt')
      .expect(403)
    expect(res.body?.success).toBe(false)
    expect(res.body?.error).toBe('ai_booking_operator_disabled')
