...
if (!project) {
  process.stderr.write('[vercel-link-project] Missing --project=<projectName>\n')
  process.exit(2)
}
...
if (res.status !== 0) {
  process.stderr.write('[vercel-link-project] Failed\n')
  if (res.stdout) process.stderr.write(res.stdout)
  if (res.stderr) process.stderr.write(res.stderr)
  process.exit(res.status ?? 1)
}

process.stdout.write('[vercel-link-project] OK\n')
