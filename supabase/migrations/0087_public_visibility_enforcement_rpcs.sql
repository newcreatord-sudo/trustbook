...
begin
  ...
  if coalesce(b.listing_visible, true) is not true and not public.is_business_member(p_business_id) then
...
begin
  ...
  into b_tz, b_lead, b_allow_ob, b_paused, b_visible, b_min_gap
  from public.businesses
  where id = p_business_id;
  ...
  if b_visible is not true and not public.is_business_member(p_business_id) then
    return;
  end if;
