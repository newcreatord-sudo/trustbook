-- Public business reputation summary (ratings + operational reliability metrics).
-- No user-private data; safe to expose to anon only when listing_visible=true.
...
grant execute on function public.get_business_public_reputation(uuid, int) to anon;
grant execute on function public.get_business_public_reputation(uuid, int) to authenticated;
