-- 0031_booking_state_transition_guard.sql
-- Centralized, validated booking/deposit state transitions.
...
begin
  ...
  if p_next_status is not null and p_next_status <> b.status then
    ...
    if b.status = 'pending_deposit' and p_next_status not in ('confirmed', 'cancelled_by_customer', 'cancelled_by_business') then
      raise exception 'invalid_transition_from_pending_deposit';
    end if;
  ...
  -- Confirm after pending_deposit requires paid state.
  if b.status = 'pending_deposit' and v_status = 'confirmed' and v_deposit <> 'paid' then
