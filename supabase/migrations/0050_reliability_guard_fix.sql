-- 0050_reliability_guard_fix.sql
-- Restore strict guards in apply_reliability_delta after anti-no-show v2 expansion.
...
begin
  ...
  if p_kind = 'rejected' and b.status <> 'rejected' then
    raise exception 'invalid_booking_state';
  end if;
  if p_kind = 'deposit_lost' and b.deposit_status <> 'forfeited' then
    raise exception 'invalid_booking_state';
  end if;
  if p_kind = 'deposit_refunded' and b.deposit_status <> 'refunded' then
    raise exception 'invalid_booking_state';
  end if;
