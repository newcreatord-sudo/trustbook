-- VIP cliente (subscription_plans.features.no_deposit_required): azzera caparra su create_booking_v3
-- quando il cliente ha abbonamento customer attivo con quel flag.

CREATE OR REPLACE FUNCTION public.customer_subscription_no_deposit_bypass(p_customer_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT auth.uid() IS NOT NULL
    AND p_customer_id = auth.uid()
    AND EXISTS (
      SELECT 1
      FROM public.customer_subscriptions cs
      INNER JOIN public.subscription_plans sp
        ON sp.id = cs.plan_id AND sp.target_audience = 'customer'
      WHERE cs.customer_id = p_customer_id
        AND cs.status = 'active'
        AND COALESCE(sp.is_active, true)
        AND COALESCE((sp.features ->> 'no_deposit_required')::boolean, false)
    );
$$;

REVOKE ALL ON FUNCTION public.customer_subscription_no_deposit_bypass(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.customer_subscription_no_deposit_bypass(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.customer_subscription_no_deposit_bypass(uuid) TO service_role;
