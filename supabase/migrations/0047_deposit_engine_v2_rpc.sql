-- Migration 0047: Update Deposit Engine RPCs
-- Upgrades compute_deposit_cents and create_booking_v2 to use the new Deposit Policy Engine.
...
BEGIN
  ...
  IF b.deposit_mode = 'none' THEN
    RETURN QUERY SELECT 0, false;
    RETURN;
  END IF;

  -- Determine risk level
  IF p_customer_effective_score < 60 THEN
    risk_level := 'red';
  ELSIF p_customer_effective_score < 75 THEN
    risk_level := 'yellow';
  ELSE
    risk_level := 'green';
  END IF;
  ...
  IF b.deposit_mode = 'everyone' THEN
    rule_type := b.deposit_value_type;
  ...
  ELSIF b.deposit_mode = 'risk_based' THEN
    IF risk_level = 'red' OR risk_level = 'yellow' THEN
      rule_type := b.deposit_value_type;
      IF rule_type = 'percentage' THEN
        rule_val := coalesce(b.deposit_percent, 0);
      ELSE
  ...
  ELSIF b.deposit_mode = 'dynamic' THEN
    ...
    ELSIF risk_level = 'yellow' THEN
      j := coalesce(b.deposit_yellow_rule, '{"type":"percentage","value":0}'::jsonb);
    ELSE
      j := coalesce(b.deposit_red_rule, '{"type":"percentage","value":0}'::jsonb);
      man_app := coalesce(b.manual_approval_for_high_risk, false);
    END IF;
    rule_type := j->>'type';
    rule_val := (j->>'value')::numeric;
  END IF;

  IF rule_val IS NULL OR rule_val <= 0 THEN
    RETURN QUERY SELECT 0, man_app;
    RETURN;
  END IF;

  IF rule_type = 'percentage' THEN
    calc_amount := round((svc_price * least(100, greatest(0, rule_val))) / 100)::int;
  ELSE
    calc_amount := rule_val::int;
  END IF;
...
-- Replace create_booking_v2 to use the new engine
...
BEGIN
  ...
  if requires_approval then
    ...
    -- User specifically requested "requires_deposit" / "pending_payment_setup" for the new deposit engine.
