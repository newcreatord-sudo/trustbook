-- Elenco pagamenti prenotazione arricchito: un'unica query per dashboard (membro) e operatore AI (dopo gate ecosistema).

create or replace function public.list_business_booking_payments_enriched(
  p_business_id uuid,
...
revoke all on function public.list_business_booking_payments_enriched(uuid, int) from public;
grant execute on function public.list_business_booking_payments_enriched(uuid, int) to authenticated;
...
begin
  ...
  return public.list_business_booking_payments_enriched(p_business_id, p_limit);
end;
$$;
