...
create index if not exists ai_suggestions_business_active_idx
on public.ai_suggestions (business_id, status, generated_at desc);

create table if not exists public.ai_suggestion_audit (
  id uuid primary key default gen_random_uuid(),
  suggestion_id uuid null references public.ai_suggestions(id) on delete set null,
  business_id uuid not null references public.businesses(id) on delete cascade,
...
create index if not exists ai_suggestion_audit_business_created_idx
on public.ai_suggestion_audit (business_id, created_at desc);
...
drop policy if exists ai_suggestions_write_none on public.ai_suggestions;
create policy ai_suggestions_write_none on public.ai_suggestions
for all to authenticated
using (false)
with check (false);

drop policy if exists ai_suggestion_audit_select_member on public.ai_suggestion_audit;
create policy ai_suggestion_audit_select_member on public.ai_suggestion_audit
for select to authenticated
using (public.is_business_member(business_id));

drop policy if exists ai_suggestion_audit_write_none on public.ai_suggestion_audit;
create policy ai_suggestion_audit_write_none on public.ai_suggestion_audit
for all to authenticated
using (false)
with check (false);

grant select on public.ai_suggestions to authenticated;
grant select on public.ai_suggestion_audit to authenticated;

revoke all on public.ai_suggestions from anon;
revoke all on public.ai_suggestion_audit from anon;
...
begin
  ...
  if not public.is_business_member(s.business_id) then
    raise exception 'not_allowed';
  end if;
