-- Privacy: evidence JSON must not embed raw UUIDs (customer/booking). Identifiers stay only in action_payload for validated RPC apply.
-- Replaces generate_ai_suggestions body from 0102 with sanitized evidence strings (logic unchanged).
...
begin
  ...
  if risky_customer_id is not null and (risky_no_show is not null and risky_no_show >= 1) then
    ...
    values (
      ...
      jsonb_build_array(
        'Profilo cliente evidenziato da regole sul periodo analizzato (identificativo tecnico solo nel payload dell''azione)',
        'No-show totali (storico affidabilità): ' || risky_no_show,
  ...
  loop
    ...
    if deliver_at > now() + interval '5 minutes' then
      ...
      values (
        ...
        jsonb_build_array(
          'Prenotazione confermata nella finestra reminder (dettaglio nel payload dell''azione)',
          'Rischio cliente: ' || row.risk_level || ' (score ' || coalesce(row.score, 80) || ')',
          'Momento promemoria proposto (server UTC): ' || to_char(deliver_at at time zone 'UTC', 'YYYY-MM-DD HH24:MI')
        ),
