...
export default function BusinessNotificationsPanel(props: {
  businessId: string
  userId: string
}) {
  ...
  useEffect(() => {
    if (!props.userId || !props.businessId) {
      setLoading(false)
      setRows([])
      return
    }
    ...
    const load = async () => {
      ...
      const [nRes, pRes] = await Promise.all([
        supabase
          .from('notifications')
          .select('*')
          .eq('recipient_user_id', props.userId)
          .eq('business_id', props.businessId)
          .or(`deliver_at.is.null,deliver_at.lte.${nowIso}`)
          .order('created_at', { ascending: false })
          .limit(50),
        supabase.from('user_preferences').select('*').eq('user_id', props.userId).maybeSingle(),
      ])
      if (!mounted) return
    ...
    void (async () => {
      try {
        ...
        if (!mounted) return
        setError(errorMessage(e, 'Errore caricamento notifiche attività.'))
      } finally {
    ...
    const ch = supabase
      .channel(`notifications_business:${props.userId}:${props.businessId}`)
      .on(
        'postgres_changes',
    ...
    <Card padded={false} className="p-6 border-white/5 bg-white/[0.02] shadow-inner">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-2 text-xs text-white/70">Aggiornamenti operativi legati alla tua attività selezionata.</div>
        </div>
        <Button
          type="button"
          variant="secondary"
      ...
      <div className="mt-6 space-y-3">
        {loading ? (
          [0, 1, 2].map((i) => <div key={i} className="h-24 w-full animate-pulse rounded-2xl bg-white/5" />)
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  {n.body ? <div className="mt-1 text-sm leading-relaxed text-white/60">{n.body}</div> : null}
